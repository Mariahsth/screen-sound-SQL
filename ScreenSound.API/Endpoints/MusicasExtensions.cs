...
public static class MusicasExtensions
{
    public static void AddEndpointsMusicas(this WebApplication app)
    {
        ...
        {
            var musica = dal.ProcurarPor(a => a.Nome.ToUpper().Equals(nome.ToUpper()));
            if (musica is null)
            {
                return Results.NotFound();
            }
        ...
        app.MapPost("/Musicas", ([FromServices] DAL<Musica> dal, [FromServices] DAL<Genero> dalGenero, [FromBody] MusicaRequest musicaRequest) =>
        {
            var musica = new Musica(musicaRequest.nome)
            {
                ...
                AnoLancamento = musicaRequest.anoDeLancamento,
        ...
        {
            var musica = dal.ProcurarPor(m => m.Id == id);
            if (musica is null)
            {
                return Results.NotFound();
            }
        ...
        {
            var musicaAAtualizar = dal.ProcurarPor(m => m.Id == musicaRequestEdit.Id);
    ...
    private static ICollection<Genero> GeneroRequestConverter(ICollection<GeneroRequest> generos, [FromServices] DAL<Genero> dalGenero)
    {
        ...
        foreach (var item in generos) 
        {
            ...
            var genero = dalGenero.ProcurarPor(g => g.Nome.ToUpper().Equals(item.Nome.ToUpper()));
